...
public class Fila_pilhaAtividade05 {
    ...
    private static Stack<Produtos>[] pilhasProduto = new Stack[NUMERO_DE_PILHAS];
    ...
    private static void adicionarProduto() {
        ...
        if (pilhasProduto[pilhaEscolhida].size() >= LIMITE_PILHA) {
        ...
        pilhasProduto[pilhaEscolhida].push(novoProduto);
    ...
    private static void retirarProduto() {
        ...
        if (pilhasProduto[pilhaEscolhida].isEmpty()) {
            System.out.println("A pilha está vazia. Nenhum produto para retirar.");
        } else {
            Produtos produtoRemovido = pilhasProduto[pilhaEscolhida].pop();
    ...
    private static void listarProdutos() {
        for (int i = 0; i < NUMERO_DE_PILHAS; i++) {
            ...
            if (pilhasProduto[i].isEmpty()) {
                ...
                for (int j = pilhasProduto[i].size() - 1; j >= 0; j--) {
                    System.out.println((pilhasProduto[i].size() - j) + ": " + pilhasProduto[i].get(j));
                }
            }
        }
    }
